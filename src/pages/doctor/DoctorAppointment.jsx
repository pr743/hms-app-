...
export default function DoctorAppointment() {
  ...
  const events = useMemo(() => {
    return appointments.map((appt) => {
      const date = new Date(appt.appointmentDate);
      const [time, modifier] = (appt.slotTime || "09:00 AM").split(" ");
      let [h, m] = time.split(":");
  ...
  return (
    <>
      ...
      <div className="p-6 bg-gray-100 min-h-screen">
        ...
        <div className="bg-white rounded-xl shadow p-4 mb-6">
          <div className="h-[600px]">
            <DnDCalendar
              ...
              endAccessor="end"
              onSelectEvent={(e) =>
                navigate(`/doctor/create-prescription?id=${e.id}`)
              }
            />
          </div>
        </div>
        ...
        {appointments.map((appt) => (
          <div key={appt._id} className="bg-white p-4 mb-3 rounded shadow">
            ...
            <button
              onClick={() =>
                navigate(`/doctor/create-prescription?id=${appt._id}`)
              }
              className="bg-blue-600 text-white px-3 py-1 mt-2 ml-2 rounded"
            >
              Open
            </button>
          </div>
