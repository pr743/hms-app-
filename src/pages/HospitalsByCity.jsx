...
function HospitalCard({ hospital, userRole, showBook = true, showSave = true }) {
  ...
  const handleSave = async () => {
    try {
      ...
      await API.post(`/patient/save-hospital/${hospital._id}`);
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: err.response?.data?.message || "Failed to save hospital",
      });
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500">
        <MapPin size={16} />
        <span>{hospital.address || "No address available"}</span>
      </div>
      ...
      <div className="flex gap-3 mt-4">
        ...
        {/* Save Button */}
        {showSave && userRole === "patient" && (
          <button
            onClick={handleSave}
            disabled={saving}
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg font-medium transition-all duration-300
              ${
                saving
        ...
        {/* Book Button */}
        {showBook && userRole === "patient" && (
          <button
            className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg font-medium shadow-md hover:shadow-lg transition-all duration-300"
          >
            📅 Book
          </button>
        )}
