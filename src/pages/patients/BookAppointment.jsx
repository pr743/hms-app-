...
function BookAppointment() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.hospitalId) {
      return Swal.fire("Error", "Select hospital", "warning");
    }

    if (!form.appointmentDate) {
      return Swal.fire("Error", "Select date", "warning");
    }

    if (!form.reason.trim()) {
      return Swal.fire("Error", "Enter reason", "warning");
    }
    ...
    try {
      ...
      Swal.fire({
        ...
        html: `
          <b>Queue Number:</b> ${res.data.queueNumber} <br/>
          <b>Slot Time:</b> ${res.data.slotTime} <br/>
          <b>Estimated Wait:</b> ${res.data.waitTime} mins
        `,
      });
      ...
      <div className="p-6 bg-gray-100 min-h-screen flex justify-center">
        <div className="bg-white w-full max-w-xl p-8 rounded-2xl shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {form.appointmentType === "emergency" && (
              <p className="text-red-500 font-semibold">
                ⚡ Emergency appointments are handled immediately (24×7)
              </p>
            )}
