...
function BookAppointment() {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ Clean validations — no slotTime check
    if (!form.doctorId) return setMessage("Please select a doctor");
    if (!form.appointmentDate)
      return setMessage("Please select appointment date");
    if (!form.reason.trim()) return setMessage("Please enter reason");
    if (loading) return;
    ...
    try {
      ...
      // ✅ Only send what backend needs — no slotTime
      const payload = {
      ...
      <div className="p-6 bg-gray-100 min-h-screen flex justify-center">
        <div className="bg-white w-full max-w-xl p-8 rounded-2xl shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* ✅ slotTime dropdown REMOVED — token assigned by server */}
            ...
            {/* ✅ Shows correct token from server */}
            {result && (
              <div className="mt-6 bg-green-50 border rounded-2xl p-4">
                <h2 className="font-bold text-green-700 text-lg mb-2">
                  ✅ Appointment Confirmed
